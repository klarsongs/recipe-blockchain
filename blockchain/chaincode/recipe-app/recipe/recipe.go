...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "closePrescription" {
		return s.closePrescription(APIstub, args)
	}
...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
	recipes := []Recipe{
		Recipe{PrescriptionID: "1", RecipeID: "1", DoctorID: "1", PatientID: "1", Medicine: "Rutinoscorbin", MedicineQuantity: "2 tabs",ExpirationDate: "2020-12-30",Note: "Take 2 tablets a day. Morning and Evening.", RecipeDate: "08-06-2017", RecipeClosed: false},
		Recipe{PrescriptionID: "2", RecipeID: "1", DoctorID: "1", PatientID: "1", Medicine: "Gripex", MedicineQuantity: "1 tab",ExpirationDate: "2020-12-30",Note: "Take once a day before going to sleep until fever stops", RecipeDate: "12-06-2018", RecipeClosed: false},
		Recipe{PrescriptionID: "3", RecipeID: "2", DoctorID: "1", PatientID: "1", Medicine: "Zabak", MedicineQuantity: "30 ml",ExpirationDate: "2020-12-30",Note: "Take 3 times a day.", RecipeDate: "21-04-2017", RecipeClosed: true}}
...
func (s *SmartContract) recordRecipe(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var recipe = Recipe{ PrescriptionID: args[0], RecipeID: args[1], DoctorID: args[2], PatientID: args[3], Medicine: args[4], MedicineQuantity: args[5], ExpirationDate: args[6], Note: args[7], RecipeDate: args[8], RecipeClosed: false}
...
func (s *SmartContract) queryPatientOpenRecipes(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	for resultsIterator.HasNext() {
		...
		// Here can modify or check data
		if recipe.PatientID != string(args[0]) || recipe.RecipeClosed {
			continue
		}
...
func (s *SmartContract) closePrescription(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	recipe.RecipeClosed = true

	recipeAsBytesMod, _ = json.Marshal(recipe)
	err := APIstub.PutState(args[0], recipeAsBytesMod)
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to close prescription: %s", args[0]))
	}
...
func (s *SmartContract) queryRecipe(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	for resultsIterator.HasNext() {
		...
		// Here can modify or check data
		if recipe.RecipeID != string(args[0]) {
			continue
		}
